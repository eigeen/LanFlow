...
设置 `LANFLOW_E2E_DISABLE_MEMORY_BUFFER=1` 可在同一数据集上关闭内存优化，执行 A/B 基线对比。

## 传输模型

- 每个对端默认 1 条控制连接、2 条数据连接，每条数据连接 4 个并发逻辑流。
- 下载器使用有界的跨文件并发流水线，小文件会轮询分配到所有数据连接；网络传输、文件创建、校验和重命名可重叠执行。
- 协议 v1.2 支持小文件批量请求。默认只将不超过 4 KiB、请求成本明显高于内容的极小文件组成约 1 MiB 的滚动内存批次，先在内存完成 BLAKE3 校验，再以有界队列落盘；协议硬限制为每批 256 个文件、8 MiB。较大文件继续并发直写，旧对端自动回退为逐文件分片请求。
- 逻辑分片默认 8 MiB，原始数据帧最多 256 KiB；控制消息走 Prost，文件数据不进入 Prost 或 WebView。
- 大目录快照持续报告扫描/哈希速度，并以约 512 KiB 的多帧分页清单返回；协议 v1.2 仍可回退兼容 v1.0/v1.1 对端。
...
- 全新大文件传输会在有限乱序缓冲区中按分片顺序计算整文件 BLAKE3，默认避免完成后再次读取目标文件；续传或高并发/超大分片配置会自动回退到落盘后整文件校验。设置页的“内存缓冲”默认 128 MiB，由极小文件批次和大文件有序哈希共享，设为 0 可完全禁用，允许范围为 0–2048 MiB。
